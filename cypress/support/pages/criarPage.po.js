...
preencherFormulariovazio(senha, confirmarSenha) {
    ...
    mensagemErro() {
        cy.get(this.box).should('have.text', 'Este e-mail já é utilizado por outro usuário.')
    }
    errosemnome(){
    ...
    errosememailnome(){
        cy.get(this.texterro).should('have.text', 'Informe seu nomeInforme seu e-mail')
    }
    erroemailinvalido(){
        cy.get(this.texterro).should('have.text', 'Formato de e-mail inválido.')
    }
    erro100(){
        cy.get(this.texterro).should('have.text', 'Informe no máximo 100 letras no seu nome')
    }
    erro60(){
        cy.get(this.box).should('have.text', 'Não foi possível efetuar seu registro. Tente novamente mais tarde.')
    }
    erroname(){
